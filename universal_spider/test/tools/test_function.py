...
class TestFunction(object):
    ...
    def test_parse_xpath(self, function: Function, htmldata, jsondata):
        ...
        xpath = '//th'
        assert function.parse_xpath(htmldata, xpath) == ['<th>编号</th>', '<th>姓名</th>', '<th>电话</th>', '<th>检测结果</th>', '<th>检测时间</th>']
        xpath = '//error'
    ...
    def test_parse_regex(self, function: Function, htmldata, jsondata):
        ...
        regex = "'(.*?)'"
        assert function.parse_regex(jsondata, regex) == ['str', 'string', 'list', 'dict', 'key', 'value', 'list_dict', 'key', 'value1', 'key', 'value2']
        regex = "编号：(.*?),"
    ...
    def test_parse_css(self, function: Function, htmldata, jsondata):
        ...
        '''
        css = 'th'
        assert function.parse_css(htmldata, css) == ['<th>编号</th>', '<th>姓名</th>', '<th>电话</th>', '<th>检测结果</th>', '<th>检测时间</th>']
        css = 'error'
