...
class BaseSpider(scrapy.Spider):
    ...
    def __init__(self, *args, **kwargs: Any):
        ...
        # 初始化
        self.stage_length = len(self._config)
        logger("init").info(f"config: {self._config}")
    ...
    async def _parse_field(self, index: int, response: Response, field_config: dict, *args, **kwargs):
        ...
        if resp_type == "browser":
            ...
            _, new_value = await replacer.replace(field_config["value"], response.meta["playwright_page"])
            return new_value
        else:
